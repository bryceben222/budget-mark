...
const Storage = {
    ...
    CATEGORIES_KEY: 'simple记账_custom_categories',
    ...
    getSettings() {
        ...
        return data ? JSON.parse(data) : { currency: '¥', darkMode: false, budget: 0 };
    },
...
const CategoryManager = {
    ...
    addCategory(type, category) {
        ...
        if (allCategories.includes(trimmedCategory)) return false;

        // 保存到本地存储
        Storage.addCustomCategory(type, trimmedCategory);
        return true;
    }
};
...
class AccountingApp {
    constructor() {
        this.records = [];
        this.settings = { currency: '¥', darkMode: false, budget: 0 };
    ...
    loadData() {
        ...
        document.getElementById('budgetInput').value = this.settings.budget || '';
    }
    ...
    bindEvents() {
        ...
        document.getElementById('editTypeSelect').addEventListener('change', (e) => {
            ...
            this.handleEditCategoryChange();
        });

        // 编辑模态框中的分类切换
        document.getElementById('editCategorySelect').addEventListener('change', () => {
            this.handleEditCategoryChange();
        });
    ...
    updateCategoryOptions(type = 'expense') {
        ...
        const categories = CategoryManager.getAllCategories(type);
    ...
    handleCategoryChange() {
        ...
        if (categorySelect.value === '其他') {
            ...
            // 使用 setTimeout 添加动画效果
            setTimeout(() => {
                customCategoryRow.classList.add('show');
                customCategoryInput.focus();
            }, 10);
        } else {
            customCategoryRow.classList.remove('show');
            setTimeout(() => {
    ...
    getFinalCategory() {
        ...
        // 如果选择了"其他"且有自定义输入
        if (category === '其他' && customCategoryInput.value.trim()) {
            const customCategory = customCategoryInput.value.trim();

            // 尝试添加自定义分类
            if (CategoryManager.addCategory(typeSelect.value, customCategory)) {
                // 更新分类选项并选中新分类
                this.updateCategoryOptions(typeSelect.value);
                categorySelect.value = customCategory;
                category = customCategory;
                this.showToast('已添加新分类');
            } else {
                // 分类已存在，直接使用
                category = customCategory;
            }
    ...
    handleEditCategoryChange() {
        ...
        const customCategoryRow = document.getElementById('editCustomCategoryRow');
    ...
    getEditFinalCategory() {
        ...
        if (category === '其他' && customCategoryInput.value.trim()) {
            ...
            if (CategoryManager.addCategory(typeSelect.value, customCategory)) {
                // 更新分类选项并选中新分类
                this.updateEditCategoryOptions(typeSelect.value);
                categorySelect.value = customCategory;
                category = customCategory;
                this.showToast('已添加新分类');
            } else {
    ...
    openEditModal(id) {
        ...
        // 填充表单数据
        document.getElementById('editTypeSelect').value = record.type;
        this.updateEditCategoryOptions(record.type);
        ...
        document.getElementById('editNoteInput').value = record.note || '';
        document.getElementById('editDateInput').value = record.date;
        document.getElementById('editCustomCategoryInput').value = '';
    ...
    saveEdit() {
        ...
        // 获取最终分类
        const category = this.getEditFinalCategory();
        ...
        if (Storage.updateRecord(this.editTargetId, updatedRecord)) {
            this.records = Storage.getRecords();
            this.render();
            document.getElementById('editModal').classList.remove('active');
            this.editTargetId = null;
    ...
    // 渲染预算
    renderBudget() {
        const budget = this.settings.budget || 0;
        const currency = this.settings.currency;
        const monthExpense = this.records
            .filter(r => Utils.isThisMonth(r.date) && r.type === 'expense')
        ...
        const percentage = Math.min((monthExpense / budget) * 100, 100);
        const remaining = Math.max(budget - monthExpense, 0);

        document.getElementById('budgetValue').textContent = 
            `${Utils.formatAmount(monthExpense, currency)} / ${Utils.formatAmount(budget, currency)}`;
                ...
                plugins: {
                    ...
                    tooltip: {
                        ...
                        callbacks: {
                            label: function(context) {
                                const value = context.raw;
                                return `${Utils.formatAmount(value, this.settings.currency)}`;
                            }.bind(this)
                        }
                    }
                }
